...
recognition.addEventListener('result', async (e) => {
  ...
  if (newSentence) {
    nextSentenceTime = getSubtitleTime(executionTime)
    newSentence = false
  }
...
startAudio.click(() => {
 let interval = setInterval(() => {
    currentSeconds++
    let percentage = parseInt((currentSeconds / audio.duration) * 100)

    percentage = `${percentage > 100 ? 100 : percentage}%`

    $('.progress-bar').css('width', percentage).html(percentage)

    if (currentSeconds >= audio.duration) {
...
processButton.click( async (e) => {
  let subArray = await parseTranslate(subtitleArray)
  processVideo(e, subArray, fileName)
})
